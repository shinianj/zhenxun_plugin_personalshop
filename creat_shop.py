    ...
    if fl == 1:
            ...
            async with db.transaction():
                ...
                if await BagUser.delete_property(event.user_id, event.group_id, name, num):
                    ...
                    if func_manager.check_send_success_message(name) and await func_text.add_goods(event.user_id,event.group_id,name,num,price):
                            sucs = True 
...
async def _(event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    async def create_shop_help() -> str:
        ...
        :return: 图片base64
        """
        goods_lst = await PersonalGoods.get_all_goods(qq,group)
        idx = 1
        ...
        for goods in _list:
            ...
            await name_image.atext(
                    (250, 0), f"{goods.goods_num}", center_type="by_height"
                )
            ...
            await goods_image.atext((15, 50), f"简介：私人商店")
...
async def _(event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    goods_list = [
        x
        for x in await PersonalGoods.get_all_goods(uid,group)
    ]
    goods_name_list = [
        x.goods_name
        for x in goods_list
    ...
    i = 0
    if msg[0] not in goods_name_list:
        await cs.finish("你的商店里没有这个商品哦",at_sender=True)
    else:
        if len(msg[1]) > 0 and is_number(msg[1]):
            try:
                if int(msg[1]) > 0:
                ...
                await cs.finish("你的奇怪数字是什么！请重新输入！！", at_sender=True)
        if f1 == 1:
            if await PersonalGoods.change_goods(uid,group,msg[0],num) :
                for _ in range(num):
                    await BagUser.add_property(uid,group,msg[0])
                    i += 1 
                if i == num:
                    await xj.finish(f'下架 {msg[0]} 共 {num} 成功！')
                else:
                    await xj.finish(f'下架 {msg[0]} 共 {num} 失败！')
            else :
                await xj.finish(f"该商品 {msg[0]} 已经清空，无法下架啦")
    ...
    if msg[0] not in goods_name_list:
        ...
        if len(msg[1]) > 0 and is_number(msg[1]):
            try:
                ...
                await cs.finish("你的奇怪数字是什么！请重新输入！！", at_sender=True)
        if f1 == 1:
            if await PersonalGoods.update_goods(uid,group,msg[0],price) :
                    await xj.finish(f'已将 {msg[0]} 的价格修改为{price}成功！')
            else :
                await xj.finish(f"该商品 {msg[0]} 已经清空，无法修改价格啦")
