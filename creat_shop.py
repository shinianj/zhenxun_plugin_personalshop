...
async def _(event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    i = 0
    if msg[0] not in goods_name_list:
        await cs.finish(f"你的商店里没有 {msg[0]} 这个商品哦",at_sender=True)
    else:
        if len(msg[1]) > 0 and is_number(msg[1]):
            try:
                if int(msg[1]) > 0:
                ...
                await cs.finish("你的奇怪数字是什么！请重新输入！！", at_sender=True)
        if f1 == 1:
            g = goods_lst[msg[0]]['num']
            if num > g :
                await xj.finish(f'超出商品上限！您的商店里明明只有 {g} 件 {msg[0]} ！')
            if await PersonalGoods.change_goods_now(uid,group,msg[0],num) :
                for _ in range(num):
                    await BagUser.add_property(uid,group,msg[0])
                    i += 1 
                if i == num:
                    await xj.finish(f'下架 {msg[0]} 共 {num} 件成功！')
                else:
                    await xj.finish(f'下架 {msg[0]} 共 {num} 件失败！')
            else :
    ...
    if msg[0] not in goods_name_list:
        ...
        if len(msg[1]) > 0 and is_number(msg[1]):
            try:
                ...
                await cs.finish("你的奇怪数字是什么！请重新输入！！", at_sender=True)
        if f1 == 1:
            if await PersonalGoods.add_goods_now(uid,group,msg[0],0,price) :
